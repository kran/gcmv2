...
- `admin.label` and `admin.group` name types in the backend. The type list inside 内容管理 showed the config keys (`article`, `supply`, …) as a flat list sorted alphabetically; a label renames them (the list shows the config key after it, in small grey) and a group splits the list, in the order the groups first appear. Types without a group collect in a leading 未分组 section; within a section the order stays the config keys'. Both are optional — a type without a label falls back to its key and one without a group stays at the top of the list rather than being filed under a catch-all — so existing `types.yaml` files keep working unchanged.
