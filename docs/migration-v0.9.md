...
Public archiving is `POST /api/nodes/{type}/{id}/archive` (it used to be `DELETE /api/nodes/{type}/{id}`): it archives instead of permanently deleting. Authenticated administrators can call `POST /admin/nodes/{id}/archive` or `/restore` with `{"revision": n}`, while `DELETE /admin/nodes/{id}` remains the explicit permanent-delete operation and can return HTTP 409 for restricted references.
