...
func TestRegisterAuth(t *testing.T) {
	s := newAuthService(t)
	id, err := s.RegisterAuth("user", "email", "a@x.com", Fields{"opaque": "credential-data"},
...
func TestRegisterAuthDupIdentifier(t *testing.T) {
	s := newAuthService(t)
	if _, err := s.RegisterAuth("user", "email", "a@x.com", Fields{"credential": "x"}, &Node{Display: "a"}); err != nil {
		t.Fatal(err)
	}
	if _, err := s.RegisterAuth("user", "email", "a@x.com", Fields{"credential": "x"}, &Node{Display: "a"}); err == nil {
	...
	// 不同类型同 identifier 允许
	if _, err := s.RegisterAuth("user", "phone", "13800138000", Fields{"credential": "x"}, &Node{Display: "a"}); err != nil {
...
func TestRegisterAuthNotAuthType(t *testing.T) {
	s := newAuthService(t)
	if _, err := s.RegisterAuth("article", "email", "a@x.com", Fields{"credential": "x"}, &Node{Display: "a"}); err == nil {
...
func TestAddRemoveAuthMethod(t *testing.T) {
	...
	// 绑定第二种方式
	if err := s.AddAuthMethod("user", id, "phone", "13800138000", Fields{"credential": "x"}); err != nil {
		t.Fatal(err)
	}
...
func TestSessionLifecycle(t *testing.T) {
	s := newAuthService(t)
	id, _ := s.RegisterAuth("user", "email", "a@x.com", Fields{"credential": "x"}, &Node{Display: "a"})
	token, err := s.CreateSession("members", id)
	if err != nil {
		t.Fatal(err)
	}
	if token == "" {
...
func TestDeleteNodeSessions(t *testing.T) {
	s := newAuthService(t)
	id, _ := s.RegisterAuth("user", "email", "a@x.com", Fields{"credential": "x"}, &Node{Display: "a"})
	first, _ := s.CreateSession("members", id)
	second, _ := s.CreateSession("members", id)
	if err := s.DeleteNodeSessions(id); err != nil {
		t.Fatal(err)
	}
