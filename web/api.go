...
func (s *Site) apiCreateNode(ctx *CmsCtx) {
	...
	event, err := ctx.writeEvent(WriteCreate, typ)
	if err != nil {
		ctx.Fail(err)
		return
	}
	...
	if err := assertAllowed(WriteCreate, typ, allowed); err != nil {
		ctx.Fail(err)
		return
	}
...
func (s *Site) apiUpdateNode(ctx *CmsCtx) {
	...
	event, err := ctx.writeEvent(WriteUpdate, typ)
	if err != nil {
		ctx.Fail(err)
		return
	}
	...
	if err := assertAllowed(WriteUpdate, typ, allowed); err != nil {
		ctx.Fail(err)
		return
	}
	if rejected := rejectFields(submitted, allowed.Items()); len(rejected) > 0 {
...
func (s *Site) apiTree(ctx *CmsCtx) {
	...
	nodes, err := ctx.ReadTree(ReadList, typ)
	if err != nil {
		ctx.Fail(err)
		return
	}
